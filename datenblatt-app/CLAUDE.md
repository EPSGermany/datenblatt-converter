...
PDF exportiert. Das PDF wird DIREKT erzeugt (reportlab, `pdf_export.py`)
- KEIN Word/LibreOffice mehr nötig, läuft ohne Installation auf jedem
PC. Bedienung über 4 Buttons in `app.py` (Tkinter-GUI): importieren,
anzeigen/kontrollieren, Export Deutsch, Export Englisch. Kein Server-/
...
`vorlage.docx`/`vorlage_en.docx` und die docx-Funktionen in `lib.py`
(`build_draft`, `convert_to_pdf`) existieren noch im Projekt, werden vom
laufenden Programm aber NICHT mehr genutzt - nur falls mal eine
editierbare Word-Version gebraucht wird.

## Dateien
```
app.py                 GUI (4 Buttons + Einstellungen)
lib.py                  Kernlogik: PDF-Extraktion, Zuordnungslogik,
                        (optional, ungenutzt) docx-Erzeugung
pdf_export.py            direkte PDF-Erzeugung (reportlab) - kein Word/
                        LibreOffice nötig, das ist der aktive Exportweg
